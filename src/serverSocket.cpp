...
bool ServerSocket::listen(size_t backlog) {
    ...
    if (!this->listening) {
        BOOST_THROW_EXCEPTION(
            syscallError() << stringInfo("listen(): Setting socket to listening.") << errcodeInfoDef());
    }
...
std::unique_ptr<IManagedSocket> ServerSocket::accept() {
    ...
    if (newSocket == -1) {
        BOOST_THROW_EXCEPTION(
            networkError() << stringInfo("accept(): accepting new socket in ServerSocket") << errcodeInfoDef());
    }

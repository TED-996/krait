...
Server::Server(ArgvConfig argvConfig)
        ...
        , serverCache(std::bind(&Server::constructPymlFromFilename, this, std::placeholders::_1, std::placeholders::_2),
...
void Server::serveClientStart() {
    ...
    try {
        while (!shutdownRequested) {
            ...
            DbgStopwatch(
                formatString("Serving request %1% %2%", httpVerbToString(requestPtr->getVerb()), requestPtr->getUrl()));
        ...
        if (isHead) {
            ...
            clientSocket->respondWithObject(
                Response(500, "<html><body><h1>500 Internal Server Error</h1></body></html>", true));
        }

...
V2PymlParser::V2PymlParser(IPymlCache& cache) : cache(cache) {
    rootItem = nullptr;
    krItIndex = 0;
}
...
void V2PymlParser::consume(std::string::iterator start, std::string::iterator end) {
    ...
    if (!stackTopIsType<PymlWorkingItem::SeqData>()) {
        ...
        BOOST_THROW_EXCEPTION(serverError() << stringInfo("Error parsing pyml file: after consuming file, working item "
                                                          "is not PymlWorkingItem::SeqData."));
    }
...
void V2PymlParser::addPymlWorkingStr(std::string&& str) {
    if (!stackTopIsType<PymlWorkingItem::SeqData>()) {
...
void V2PymlParser::addPymlWorkingPyCode(PymlWorkingItem::Type type, std::string&& code) {
...
void V2PymlParser::addPymlWorkingEmbed(std::string&& filename) {
...
void V2PymlParser::addPymlWorkingCtrl(std::string&& ctrlCode) {
    if (!stackTopIsType<PymlWorkingItem::SeqData>()) {
        BOOST_THROW_EXCEPTION(serverError() << stringInfo("Pyml FSM error: stack top not Seq."));
    }
...
void V2PymlParser::pushPymlWorkingIf(std::string&& condition) {
...
void V2PymlParser::addCodeToPymlWorkingFor(int where, std::string&& code) {
    if (!stackTopIsType<PymlWorkingItem::ForData>()) {
        BOOST_THROW_EXCEPTION(
            serverError() << stringInfo("Parser error: tried to add code to <?for ?> without a for being on top"));
    }
    if (code.length() == 0 || isWhitespace(code)) {
        return;
    }
...
void V2PymlParser::addPymlStackTop() {
    ...
    *newItem = itemSrc;
    ...
    if (!stackTopIsType<PymlWorkingItem::SeqData>()) {
        BOOST_THROW_EXCEPTION(
            serverError() << stringInfoFromFormat("Pyml FSM error: stack top not Seq, but %1%", itemStack.top().type));
    }
    PymlWorkingItem::SeqData& data = getStackTop<PymlWorkingItem::SeqData>();
...
void V2PymlParserFsm::init() {
    ...
    // Finish @{...}
    add(execRoot,
        new PymlAddPyCodeTransition(&parser,
            new Skip(new Condition(
                new Simple('}', start), [=](char chr, FsmV2& fsm) { return fsm.getProp(bracketDepthKey) == 0; })),
    ...
    // Decrease depth
    add(execRoot,
        new Action(new Condition(new Simple('}', execRoot),
    ...
    add(evalRawRoot,
        new PymlAddPyCodeTransition(&parser, new OrFinal(new Whitespace(start)), PymlWorkingItem::PyEvalRaw));
    // Finish by @...@
    add(evalRawRoot,
        new PymlAddPyCodeTransition(&parser, new Skip(new Simple('@', start)), PymlWorkingItem::PyEvalRaw));
    // Escape strings
    ...
    addFinalActionToMany(
        [=](FsmV2& fsm) {
            finalThrowError(pymlError() << stringInfo(
                                "Incomplete '@...'/'@!...' Python expression, most likely unfinished string literal"));
        },
        {evalRoot, evalRawRoot});
    addFinalActionToMany(
        [=](FsmV2& fsm) { finalThrowError(pymlError() << stringInfo("Unterminated '@{ ... }' block.")); }, {execRoot});
    addFinalActionToMany(
        [=](FsmV2& fsm) { finalThrowError(pymlError() << stringInfo("Incomplete '@if ... :' instruction.")); },
        {ifRoot});
    addFinalActionToMany(
        [=](FsmV2& fsm) { finalThrowError(pymlError() << stringInfo("Incomplete '@else:' instruction.")); },
        {elseRoot});
    addFinalActionToMany(
        [=](FsmV2& fsm) { finalThrowError(pymlError() << stringInfo("Incomplete '@for ... in ... :' instruction.")); },
        {forEntry, forPreIn, forPostIn, forCollection});
    addFinalActionToMany(
        [=](FsmV2& fsm) { finalThrowError(pymlError() << stringInfo("Incomplete '@import ...' instruction.")); },
        {importRoot});
    addFinalActionToMany(
        [=](FsmV2& fsm) {
            BOOST_THROW_EXCEPTION(
                serverError() << stringInfoFromFormat("BUG IN PYML PARSER: stuck in final state %1%", fsm.getState()));
        },
...
void V2PymlParserFsm::finalAddPymlStr() {
    ...
    if (pymlStr.length() != 0) {
        parser->addPymlWorkingStr(std::move(pymlStr));
    }
}

...
public:
    explicit _DbgStopwatch(std::string&& name) : name(std::move(name)) {
        restart();
    }
    ...
    void stopPrint() {
        if (running) {
            ...
            _DBG_FMT(
                "[DbgStopwatch] [%1%] %2%ms", name, std::chrono::duration<double, std::milli>(stop - start).count());
            running = false;
        }
    }
...
public:
    ...
    void stopPrint() {
        if (total.count() != 0) {
            _DBG_FMT("[DbgStopwatch] [%1%] %2%ms", name, std::chrono::duration<double, std::milli>(total).count());
            total = duration();
        }
    }
...
public:
    _DbgDependentStopwatch(Aggregator& aggregator) : aggregator(aggregator) {
        restart();
    }
...
#define DbgDependentStopwatch(aggregator) DbgDependentStopwatchVar(depStopwatch, (aggregator))

...
OpenSslManagedSocket::OpenSslManagedSocket(int socket, SSL_CTX* ctx) : ManagedSocket(socket), ssl(SSL_new(ctx)) {
...
void OpenSslManagedSocket::initialize() {
    if (ssl == nullptr) {
        BOOST_THROW_EXCEPTION(
            serverError() << stringInfo("OpenSslManagedSocket::initialize: Managed socket used after context detach."));
    }

    if (SSL_accept(ssl) <= 0) {
        BOOST_THROW_EXCEPTION(
            sslError() << stringInfo("SSL_accept: performing SSL handshake") << SslUtils::getSslErrorInfo());
    }
}
...
void OpenSslManagedSocket::atFork() {
    if (ssl == nullptr) {
        BOOST_THROW_EXCEPTION(
            serverError() << stringInfo("OpenSslManagedSocket::atFork: Managed socket used after context detach."));
    }
...
int OpenSslManagedSocket::write(const void* data, size_t nBytes, int timeoutSeconds, bool* shouldRetry) {
    if (ssl == nullptr) {
        BOOST_THROW_EXCEPTION(
            serverError() << stringInfo("OpenSslManagedSocket::write: Managed socket used after context detach."));
    }
...
int OpenSslManagedSocket::read(void* destination, size_t nBytes, int timeoutSeconds, bool* shouldRetry) {
    if (ssl == nullptr) {
        BOOST_THROW_EXCEPTION(
            serverError() << stringInfo("OpenSslManagedSocket::read: Managed socket used after context detach."));
    }

...
static std::unordered_map<int, std::string> statusReasons{{101, "Switching Protocols"},
    {200, "OK"},
    {304, "Not Modified"},
    {400, "Bad Request"},
    {401, "Unauthorized"},
...
Response::Response(int httpMajor,
    int httpMinor,
    int statusCode,
...
Response::Response(int statusCode, std::string&& body, bool connClose) : bodyIterator(std::move(body)) {
    this->httpMajor = 1;
    this->httpMinor = 1;
    this->statusCode = statusCode;
    this->fromFullResponse = false;
...
Response::Response(const std::string& fullResponse) : bodyIterator("") {
...
std::string Response::getResponseHeaders() const {
    ...
    if (fromFullResponse) {
        ...
        statusLine
            = (b::format("HTTP/%1%.%2% %3% %4%") % httpMajor % httpMinor % statusCode % getStatusReason(statusCode))
                  .str();
    }

...
int main(int argc, char* argv[]) {
    ...
    try {
        bpo::store(
            bpo::command_line_parser(argc, argv).options(cmdlineOptions).positional(positionalDesc).run(), parsedArgs);
        ...
        std::cerr << "Invalid arguments: missing option " << e.get_option_name() << std::endl

    ...
    while (pollResult != 0) {
        if (pollResult == -1) {
            BOOST_THROW_EXCEPTION(syscallError()
                << stringInfoFromFormat("poll(): on fd %1% (to check if closed)", fd) << errcodeInfoDef());
        }
        ...
        if (readResult == -1) {
            ...
            if (errno != EAGAIN && errno != EWOULDBLOCK) {
                BOOST_THROW_EXCEPTION(syscallError()
                    << stringInfoFromFormat("read(): on fd %1% (to check if closed)", fd) << errcodeInfoDef());
            }
        }
        if (readResult == 0) {
            return true;
...
std::string pyErrAsString() {
    if (PyErr_Occurred() == nullptr) {
        BOOST_THROW_EXCEPTION(
            serverError() << stringInfo("Tried to call pyErrAsString() when no Python exception occured."));
    }

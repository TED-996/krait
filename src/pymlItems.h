...
public:
    PymlItemSeq(std::vector<std::unique_ptr<const IPymlItem>>&& items) : items(std::move(items)) {
    }
...
public:
    PymlItemIf(const std::string& conditionCode,
        std::unique_ptr<const IPymlItem>&& itemIfTrue,
        std::unique_ptr<const IPymlItem>&& itemIfFalse)
...
public:
    PymlItemFor(
        const std::string& entryName, const std::string& collection, std::unique_ptr<const IPymlItem>&& loopItem)
            : entryName(entryName), collection(collection), loopItem(std::move(loopItem)) {
        static unsigned int krItIndex = 0;
        std::string iterName = (formatString("_krIt%d", krItIndex++));

        initCode = formatString(
            "%1% = IteratorWrapper(%2%)\nif not %1%.over: %3% = %1%.value", iterName, collection, entryName);
...
public:
    PymlItemEmbed(std::string filename, IPymlCache& cache) : filename(filename), cache(cache) {
    }
...
public:
    PymlItemSetCallable(const boost::python::object& callable, const std::string& destination)
            : callable(callable), destination(destination) {
    }
    ...
    CodeAstItem getCodeAst() const override {
        BOOST_THROW_EXCEPTION(serverError()
            << stringInfo("PymlItemSetCallable::getCodeAst() called without canConvertToCode being called."));
    }
    bool canConvertToCode() const override {
        return false;
    }

...
SimpleFsmTransition::SimpleFsmTransition(char chrToMatch, size_t nextState, bool consume)
        : chrToMatch(chrToMatch), nextState(nextState), consume(consume) {
}
...
FsmV2::FsmV2(size_t nrStates, size_t nrBulkStates, bool defaultSkip)
        : maxState(nrStates), maxBulkState(nrStates + nrBulkStates), currBulkState(nrStates), defaultSkip(defaultSkip) {
...
void FsmV2::addFinalAction(size_t state, const FsmV2::fsmAction& action) {
    if (state >= maxState) {
        BOOST_THROW_EXCEPTION(
            serverError() << stringInfo("Requested state to add final action to larger than maximum."));
    }
...
void FsmV2::addToBulk(size_t state, FsmTransition*&& transition) {
    if (state >= maxBulkState) {
        BOOST_THROW_EXCEPTION(
            serverError() << stringInfo("Requested bulk state to add transition to larger than maximum."));
    }
...
void FsmV2::addBlockParser(size_t startState, size_t endState, char blockStart, char blockEnd) {
    ...
    // Decrease depth
    addToBulk(consumeState,
        new AndConditionFsmTransition(
            new ActionFsmTransition(new SimpleFsmTransition(blockEnd, consumeState),
                [=](FsmV2& fsm) { fsm.setProp(depthPropName, fsm.getProp(depthPropName) - 1); }),
            [=](char ch, FsmV2& fsm) { return fsm.getProp(depthPropName) != 0; }));
    // Finish block
    addToBulk(consumeState,
        new AndConditionFsmTransition(
            new ActionFsmTransition(new SimpleFsmTransition(blockEnd, endState),
                [=](FsmV2& fsm) { fsm.setProp(depthPropName, fsm.getProp(depthPropName) - 1); }),
            [=](char ch, FsmV2& fsm) { return fsm.getProp(depthPropName) == 0; }));

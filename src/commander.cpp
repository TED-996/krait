        ...
        if (!bf::create_directory(fifoNameBase)) {
            BOOST_THROW_EXCEPTION(syscallError()
                << stringInfoFromFormat(
                       "mkdir(): creating command directory %1%. Check that you have appropriate rights.",
                       fifoNameBase.string())
                << errcodeInfoDef());
        }
    }
    ...
    if (!bf::exists(fifoName)) {
        if (create) {
            if (!mkfifo(fifoName.c_str(), 0666)) {
                BOOST_THROW_EXCEPTION(syscallError()
                    << stringInfoFromFormat("mkfifo(): creating command file (%1%)", fifoName) << errcodeInfoDef());
            }
        } else {
            return -1;
        }
    ...
    if (fifoFd < 0) {
        if (errno == ENXIO) {
            BOOST_THROW_EXCEPTION(
                cmdrError() << stringInfoFromFormat("There is no running server listening on command file."));
        } else {
            BOOST_THROW_EXCEPTION(syscallError()
                << stringInfoFromFormat("open(): opening command file (%1%)", fifoName) << errcodeInfoDef());
        }
    }
    if (fcntl(fifoFd, F_SETFD, flags) < 0) {
        BOOST_THROW_EXCEPTION(syscallError()
            << stringInfoFromFormat("fcntl(): opening command file (%1%)", fifoName) << errcodeInfoDef());
    }
...
void commanderStart(pid_t mainPid, int fifoFd) {
    ...
    while (getppid() == mainPid) {
        ...
        if (pollResult == 1) {
            ...
            if (bytesRead == -1) {
                BOOST_THROW_EXCEPTION(
                    syscallError() << stringInfo("read(): reading command fifo.") << errcodeInfoDef());
            } else if (bytesRead == 0) {
                ...
                if (!processCommand(mainPid, cmdBuffer, bytesRead)) {
                    BOOST_THROW_EXCEPTION(cmdrError()
                        << stringInfoFromFormat("Execution of command %1% failed.", std::string(cmdBuffer, bytesRead)));
                }
            }
        }
    }

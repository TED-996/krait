...
const Route& Route::getRouteMatch(const std::vector<Route>& routes,
    RouteVerb verb,
    ...
    BOOST_THROW_EXCEPTION(routeError() << stringInfoFromFormat(
                              "Error: Could not match url %1% on method %2% with any route. Is there no default route?",
                              url,
                              routeVerbToString(verb)));
}
